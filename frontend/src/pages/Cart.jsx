...
function Cart() {
  ...
  const fetchPerfumesDetails = async (ids) => {
    try {
      const requests = ids.map(id => axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/product/${id}`));
  ...
  if (token) {
    try {
      ...
      await axios.post(
        `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/cart`, 
...
const handleDelete = (id) => {
  ...
  decreaseNumber(1); // Update cart count in context
};
